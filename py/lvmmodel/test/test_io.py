...
#
# Try to load the LVMMODEL environment variable
#
lvmmodel_available = True
lvmmodel_message = "The lvmmodel data set was not detected."
try:
    spam = os.environ['LVMMODEL']
except KeyError:
    lvmmodel_available = False
    specter_available = False
    specter_message = lvmmodel_message
...
class TestIO(unittest.TestCase):
    ...
    @classmethod
    def setUpClass(cls):
        global specter_available, lvmmodel_available
        cls.specter_available = specter_available
        cls.lvmmodel_available = lvmmodel_available
    ...
    def setUp(self):
        """Ensure that any lvmmodel.io caches are clear before running
        any test.
        """
        io.reset_cache()
    ...
    @unittest.skipUnless(lvmmodel_available, lvmmodel_message)
    ...
    @unittest.skip('Skip *until* the pixel weights file is in the LVMMODEL directory')
#    @unittest.skipUnless(lvmmodel_available, lvmmodel_message)
    ...
    def test_load_tiles_alt(self):
        ...
        #- no path; should fail since that file isn't in $LVMMODEL/data/footprint/
    ...
    def test_trim_data(self):
        ...
        if os.path.getsize(psffile) > 20e6:
            from .. import trim
            indir = os.path.join(os.getenv('LVMMODEL'), 'data')
