...
def generate_random_centroid_offsets(rms_offset=default_offset, seed=123):
    ...
    The arrays in the files $LVMMODEL/data/throughput/DESI-0347_random_offset_<n>.fits
...
def get_tile_radius_mm():
    ...
    if _tile_radius_mm is None:
        import lvmmodel.io
        fp = lvmmodel.io.load_fiberpos()
        p = lvmmodel.io.load_desiparams()
...
def get_tile_radius_deg():
    ...
    if _tile_radius_deg is None:
        ...
        platescale = lvmmodel.io.load_platescale()
...
class FocalPlane(object):
    ...
    def __init__(self, ra=0.0, dec=0.0):
        """
        """
        # Read $LVMMODEL/data/focalplane/fiberpos.fits and platescale.txt
        # to construct focal plane model.  May also need data/desi.yaml .
        self._check_radec(ra, dec)
        self.ra = ra
        self.dec = dec
        self._fiberpos_file = os.path.join(os.environ['LVMMODEL'],
                                           'data', 'focalplane',
                                           'fiberpos.fits')
...
def fiber_area_arcsec2(x, y):
    ...
    '''
    from lvmmodel.io import load_desiparams, load_platescale
...
def on_gfa(telra, teldec, ra, dec, buffer_arcsec = 100):
    ...
    lvmmodel.footprint.find_points_in_tiles(tiles, ra, dec, radius) with this function to
    ...
    inrangeindices = lvmmodel.footprint.find_points_radec(telra, teldec, ra, dec, 1.65)
    if not inrangeindices:
        return np.array([]), np.array([])
    inrangeindices = np.asarray(inrangeindices)

    targetx, targety = lvmmodel.focalplane.radec2xy(telra, teldec, ra[inrangeindices], dec[inrangeindices])
...
def _retrieve_minimum_boundary(x_tolerance, y_tolerance):
    ...
    #x, y = lvmmodel.focalplane.radec2xy(7.11, -10.53, targetx, targety)
    # If any calculated area is under the threshold area, it is mathematically impossible
    THRESHOLD_AREA = 469.7
    MIN_TOLERANCE = 0.001
    # The area boundary's value is the area of the gfa plus some tolerance.
...
def on_tile_gfa(tileid, targets, buffer_arcsec = 100):
    ...
    """
    import lvmmodel.footprint
    telra, teldec = lvmmodel.footprint.get_tile_radec(tileid)
...
def get_gfa_targets(targets, rfluxlim = 1000, tiles = None, buffer_arcsec = 100):
    ...
    tiles: table of tiles, default to lvmmodel.io.load_tiles()
    buffer_arcsec: (float) additional buffer region around GFA to include
    ...
    import lvmmodel.footprint
    points = lvmmodel.footprint.find_points_in_tiles(tiles, targets['RA'], targets['DEC'])
    alltargetindices = []
    tileidlist = []
    gfaidlist = []
