...
def svn_export(lvmmodel_version=None):
    """Create a :command:`svn export` command suitable for downloading a
    particular lvmmodel version.

    Parameters
    ----------
    lvmmodel_version : :class:`str`, optional
    ...
    if lvmmodel_version is None:
        export_version = 'trunk'
    elif lvmmodel_version is 'trunk' or 'branches/' in lvmmodel_version:
        export_version = lvmmodel_version
    else:
        export_version = 'tags/' + lvmmodel_version
    return ["svn", "export",
            ("https://desi.lbl.gov/svn/code/lvmmodel/" +
             "{0}/data").format(export_version)]


def install(lvmmodel=None, version=None):
    """Primary workhorse function.

    Parameters
    ----------
    lvmmodel : :class:`str`, optional
        Allows the install directory to be explicitly set.
    version : :class:`str`, optional
        Allows the lvmmodel version to be explicitly set.
    ...
    try:
        install_dir = environ['LVMMODEL']
    except KeyError:
        if lvmmodel is not None:
            install_dir = lvmmodel
        else:
...
def main():
    """Entry point for the :command:`install_lvmmodel_data` script.
    ...
    desc = """Install lvmmodel data.

This script will attempt to download and install the lvmmodel data/ directory.
The script will attempt to attempt to install the data in the following
locations, in order of preference:

1. :envvar:`LVMMODEL`, that is, the directory specified by the
...
""".format(default_install_dir())
    ...
    parser.add_argument('-d', '--lvmmodel', action='store', dest='lvmmodel',
                        metavar='LVMMODEL',
                        help=('Place the data/ directory in this directory. ' +
                              'In other words, the environment variable ' +
                              'LVMMODEL should be set to this directory.'))
    parser.add_argument('-D', '--lvmmodel-version', action='store',
                        dest='lvmmodel_version', metavar='VERSION',
                        help='Explicitly set the version to download.')
    options = parser.parse_args()
    try:
        install(options.lvmmodel, options.lvmmodel_version)
