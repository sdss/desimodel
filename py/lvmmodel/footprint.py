...
def tiles2pix(nside, tiles=None, radius=None, per_tile=False, fact=4):
    ...
    Optional:
        tiles:
            ...
            None to use all DESI tiles from lvmmodel.io.load_tiles()
        radius: tile radius in degrees;
            if None use lvmmodel.focalplane.get_tile_radius_deg()
    ...
    if radius is None:
        import lvmmodel.focalplane
        radius = lvmmodel.focalplane.get_tile_radius_deg()
...
def tiles2fracpix(nside, step=1, tiles=None, radius=None, fact=4):
    ...
    pix = lvmmodel.footprint.tiles2pix(nside, tiles=tiles, radius=radius, fact=fact)
    ...
    verts_in = lvmmodel.footprint.is_point_in_desi(tiles, ra, dec, radius=radius)
...
def pixweight(nside, tiles=None, radius=None, precision=0.01, outfile=None, outplot=None):
    ...
    Notes:
        ...
        lvmmodel.io.load_pixweight() can downsample the array to lower nsides
    '''
    t0 = time()
    ...
    pix = lvmmodel.footprint.tiles2pix(nside, tiles=tiles, radius=radius, fact=2**8)
    weight[pix] = 1.
    ...
    for i in range(20):
        ...
        fracpix = lvmmodel.footprint.tiles2fracpix(
    ...
    indesi = lvmmodel.footprint.is_point_in_desi(lvmmodel.io.load_tiles(),rainmask,decinmask)
    ...
    if outfile is not None:
        #ADM get path to LVMMODEL footprint directory, create output file name
        import lvmmodel.io

        #ADM write information indicating HEALPix setup to file header
        #ADM include lvmmodel version as a check in case footprint changes
        import fitsio
        from lvmutil import depend

        hdr = fitsio.FITSHDR()
        depend.setdep(hdr, 'lvmmodel', lvmmodel_version)
...
def pix2tiles(nside, pixels, tiles=None, radius=None):
    ...
    ii = lvmmodel.footprint.find_tiles_over_point(tiles, ra, dec, radius=radius+pixsize)
...
def is_point_in_desi(tiles, ra, dec, radius=None, return_tile_index=False):
    ...
    default radius is from lvmmodel.focalplane.get_tile_radius_deg()
    """
